    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, (self.max_n_components + 1)):
        	try:
        		...
        		n_paras = n_components * (n_components - 1) + 2 * n_components * n_features
        		logN = np.log(self.X.shape[0])
        		BIC = -2 * logL + n_paras * logN
...
class SelectorDIC(ModelSelector):
    ...
    def generate_dictionary(cls, inst):
        ...
        for n_components in range(inst.min_n_components, (inst.max_n_components + 1)):
            ...
            for word in inst.words.keys():
                ...
                try:
                    model = GaussianHMM(n_components = n_components, covariance_type="diag", n_iter=1000, random_state = inst.random_state, verbose=False).fit(X, lengths)
    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, (self.max_n_components + 1)):
            ...
            models = SelectorDIC.models[n_components]
            vals =  SelectorDIC.values[n_components]
            if(self.this_word not in vals):
                continue
            
            avg = np.mean([vals[word] for word in vals.keys() if word != self.this_word])
            DIC = vals[self.this_word] - avg
            
            if DIC > best_score:
                best_score = DIC
                best_model = models[self.this_word]
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components + 1):
            scores = []
            n_split = SelectorCV.n_split
            model, logL = None, None
            
            if(len(self.sequences) < n_split):
                break
            
            split_method = KFold(random_state = self.random_state, n_splits = n_split)
